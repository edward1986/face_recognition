...
class ItemImageSaved
{
    ...
    const THRESHOLD = 20; // What percentage of colors should be saved?
    const BASE_COLORS = [// Reference color
        'red' => [128, 0, 0],
        'blue' => [0, 0, 128],
        'yellow' => [128, 128, 0],
        'green' => [0, 128, 0],
    ...
    public function __construct(ItemImage $item_image)
    {
        ...
        foreach ($palette as $color => $count) {
            ...
            $extracted_rgb = array_values(Color::fromIntToRgb($color));
            $min_distance = 765; // Start from maximum distance
            $color_key = '';
        ...
        foreach ($extracted_color_counts as $color_key => $count) {
            ...
            if ($count > 0 && in_array($color_key, ['red', 'blue', 'yellow', 'green'])) {
                ...
                if ($percentage > self::THRESHOLD) {// Save if greater than threshold
    ...
    function getColorDistance($color_1, $color_2)
    {// Get how far the two colors are apart
